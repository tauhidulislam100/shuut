  ...
  return (
    <>
      <div className="w-full mt-[60px] flex justify-end gap-2">
        <button className="px-7 py-2.5 bg-[#FCFCFD] border-[0.5px] border-[#D0CFD84D] rounded-md font-lota">
          Check Dates
        </button>
        <button className="px-7 py-2.5 bg-[#FCFCFD] border-[0.5px] border-[#D0CFD84D] rounded-md font-lota">
          Today
      ...
      <div className="">
        ...
        <div className="grid grid-cols-4">
          {tripodInLagos.map((itm, idx) => (
            <SingleProduct data={itm as any} key={`today_${idx}`} />
          ))}
        </div>
      </div>
    </>
